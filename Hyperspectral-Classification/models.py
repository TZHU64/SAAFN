...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
        ...
        lr = kwargs.setdefault('learning_rate', 0.01)
        center_pixel = True
        model = ASPP(n_bands, n_classes, patch_size=kwargs['patch_size'])
        # For Adagrad, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adagrad(model.parameters(), lr=lr, weight_decay=0.01)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
...
class ASPP(nn.Module):
    """
    3D Atrous Spatial Pyramid Pooling Network
    """
    @staticmethod
    def weight_init(m):
        if isinstance(m, nn.Linear) or isinstance(m, nn.Conv3d):
    ...
    def __init__(self, input_channels, n_classes, patch_size=7):
        ...
        self.patch_size = patch_size
        dr=15
        self.conv1= nn.Conv3d(1, 16, (1, 1, 1))
        self.conv2_1 = nn.Conv3d(16, 16, (1, 1, 1))
        self.conv2_2 = nn.Conv3d(16, 16, (3, 3, 3), padding=(dr,dr,dr), dilation=(dr,dr,dr))
        self.conv2_3 = nn.Conv3d(16, 16, (3, 3, 3), padding=(dr*2,dr*2,dr*2), dilation=(dr*2,dr*2,dr*2))
        self.conv2_4 = nn.Conv3d(16, 16, (3, 3, 3), padding=(dr*3,dr*3,dr*3), dilation=(dr*3,dr*3,dr*3))
        self.conv4 = nn.Conv3d(16, 16, (3, 3, 3))
        self.pooling = nn.MaxPool2d((3, 3, 3),stride=(3,3,3))
